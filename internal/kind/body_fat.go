...
// bodyFatRecord は体脂肪率の保存用モデル。単位変換は要らない。
...
type bodyFatKind struct{}

func (bodyFatKind) Key() string { return "body_fat" }

func (bodyFatKind) Policy() Policy {
	return Policy{Window: WindowAll, Daily: []string{FuncMean, FuncMin, FuncMax, FuncCount}}
}

func (bodyFatKind) Table() cumdb.Table {
	return envelopeTable("body_fat", cumdb.Column{Name: "percent", Type: "REAL"})
}

func (k bodyFatKind) ExportRows(ctx context.Context, exportDB *sql.DB) ([][]any, []string, error) {
	read, err := hcsql.ReadInstant(ctx, exportDB, "body_fat_record_table", []string{"percentage"})
	if err != nil {
		return nil, nil, err
	}
	...
	for _, r := range read {
		rec := bodyFatRecord{UUID: r.UUID, Time: r.Time, ZoneOffset: r.ZoneOffset, AppID: r.AppID}
		if r.Values[0].Valid {
...
func (k bodyFatKind) load(ctx context.Context, cum *sql.DB, sinceMs int64) ([]bodyFatRecord, error) {
	return loadAll(ctx, cum, k.Table(), sinceMs, func(rows *sql.Rows) (bodyFatRecord, error) {
		var rec bodyFatRecord
		var endTime int64
...
func (k bodyFatKind) Aggregate(ctx context.Context, cum *sql.DB) ([]model.AggRecord, error) {
...
func (bodyFatKind) ValueNames() []string { return []string{"percent"} }

func (k bodyFatKind) RawHeader() []any { return rawHeader(k.ValueNames()...) }

func (k bodyFatKind) RawRows(ctx context.Context, cum *sql.DB, sinceMs int64) ([][]any, error) {
