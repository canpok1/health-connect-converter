# Architecture Decision Records (ADR)

重要度の高い判断を記録する。フォーマットは MADR 軽量版（日本語本文）。

| 番号 | タイトル | ステータス | 日付 |
|------|----------|------------|------|
| [0001](0001-distribute-container-image-via-ghcr.md) | コンテナイメージをGHCR経由で配布する | 採用 | 2026-08-29 |
| [0002](0002-bundle-config-into-image.md) | config.yamlをコンテナイメージに同梱する | 廃止 | 2026-08-29 |
| [0003](0003-relative-data-volume-path.md) | 累積データの保存先を相対パス（gitクローン先内）にする | 採用 | 2026-08-29 |
| [0004](0004-config-driven-generic-records.md) | 種別を汎用レコードで扱い、累積DBのスキーマをconfig.yamlから生成する | 置換 | 2026-08-29 |
| [0005](0005-deployment-identifiers-via-env-file.md) | デプロイ固有の識別子は.envの環境変数で渡す | 採用 | 2026-08-29 |
| [0006](0006-fix-container-uid-to-1000.md) | コンテナの実行UID/GIDをplant-diaryに合わせて固定する | 採用 | 2026-08-30 |
| [0007](0007-daily-summary-as-first-tab.md) | Claudeに読ませる正本としてdaily_summaryを先頭タブに固定する | 採用 | 2026-08-30 |
| [0008](0008-replace-cumulative-records-per-day.md) | エクスポートに含まれる日は累積DBを日ごと置き換える | 採用 | 2026-09-03 |
| [0009](0009-dedupe-by-health-connect-app-priority.md) | 日次集約をGo側で行い、アプリ優先度で重複排除する | 採用 | 2026-09-03 |
| [0010](0010-force-ingest-on-startup.md) | 起動後の最初の1周は新着ZIPの有無によらず取り込む | 採用 | 2026-09-12 |
| [0011](0011-sleep-stage-with-own-times.md) | 睡眠ステージはステージ自身の時刻で持ち、起床日に数える | 採用 | 2026-09-12 |
| [0012](0012-split-measured-data-from-aggregation-model.md) | 実測データと集計モデルを分け、実測データは種別ごとの型で表す | 採用 | 2026-09-12 |
