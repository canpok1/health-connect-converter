...
ZIP の `modifiedTime` が前回処理分より新しいときだけ処理する。新着がなければ Drive 1回・Sheets 1回（タブ順の是正）の計2呼び出しで即終了するので、ポーリング頻度を上げてもコストはない。

**ただしプロセス起動後の最初の1周だけは、新着の有無によらず最新ZIPを取り込む**（[ADR 0010](../adr/0010-force-ingest-on-startup.md)）。種別を追加しても累積 DB に当該データが無いため、新着時のみに徹すると次のエクスポート（最大24時間後）まで出力に現れない。Watchtower はイメージ更新時にコンテナを作り直すので、起動を設定・コード変更の合図として使う。`--once` は毎回プロセスが起動するため常に取り込む。

`modifiedTime` は**ミリ秒まで保存する**。秒精度で保存すると復元値が常に実際より古くなり、同じZIPが毎周回「新着」と判定される（2026-09-12 まで実際にそうなっていた）。
...
ただし Claude が CSV 経由で読む場合は先頭タブしか見えず `_meta` に届かないため、**`daily_summary` の最終行の日付**を鮮度の指標にする。歩数は毎日記録されるので、最終行が数日以上古ければ取り込みが止まっている（[ADR 0007](../adr/0007-daily-summary-as-first-tab.md)）。
