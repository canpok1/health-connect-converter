...
```
エクスポートDB
  │  種別ごとの読み出し（表の写し。JOINも単位変換もしない）
  ▼
① ヘルスコネクトのモデル ── 種別ごと。上流の表に1対1で対応
  │  種別ごとの変換（親子を畳む・アプリ名を引く・単位を直す）
  ▼
...
**層を分ける目的は、変更の理由を分けること。** 上流の表が変われば①だけ、保存の都合なら②だけ、集計の都合なら③④だけが変わる。

### ① ヘルスコネクトのモデル

エクスポートDBの表をそのまま写す。**判断を持たせない**（JOIN も単位変換も捨てる列の選別もしない）。種別によっては親子2つの表に対応する。

### ② 保存用のモデル

累積DBに保存する形。①から次を済ませる。

- 親子を畳む（心拍・速度は「1行＝1測定値」、睡眠ステージは「1行＝1区間」）
...
`列名←元の列` の形で、②が持つ値と①からの対応を示す。倍率は単位変換。

| 種別キー | ①の元テーブル | 時刻の形 | ②が持つ値 | 現地日 | 重複排除 | カテゴリ | 生データの窓 | 日次 |
|---|---|---|---|---|---|---|---|---|
...
ヘルスコネクトは内部表現で保持しており、そのままでは読めない値がある。②へ変換するときに直す。
